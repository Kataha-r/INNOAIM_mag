...
function setLastDeletedProduct(value) {
  ...
  try {
    if (value) localStorage.setItem(LAST_DELETED_KEY, JSON.stringify(value));
  ...
  productsList.innerHTML = visibleProducts.length
    ? visibleProducts.map((product) => {
      ...
      <div class="product-list-row" data-product-id="${product.id}" role="button" tabindex="0" aria-label="Otwórz produkt ${escapeHtml(product.name)}">
        ...
        <span class="row-name ${Number(product.stock) < Number(product.required) ? "low-stock-name" : ""}">
...
function renderStatistics() {
  ...
  statisticsProductsList.innerHTML = products.map((product, index) => `
    <div class="statistics-product-row">
      <span class="row-number">${index + 1}</span>
      <span class="row-image"><img src="${escapeHtml(getProductImages(product)[0] || DEFAULT_IMAGE)}" alt="" /></span>
      <button class="statistics-product-name ${Number(product.stock) < Number(product.required) ? "low-stock-name" : ""}" type="button" data-statistics-product="${product.id}">
...
function setView(view) {
  ...
  pageTitle.textContent = showStatistics
    ? "Statystyki wysyłek"
    : (showList ? "Wszystkie produkty" : "Szczegóły produktu");
  allProductsButton.classList.toggle("active", showList);
  productsNavButton.classList.toggle("active", !showStatistics);
...
async function undoLastProductDeletion() {
  ...
  if (products.length >= 100) {
    showToast("Najpierw usuń inny produkt — osiągnięto limit 100 produktów.");
    return;
  }
...
form.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    setSyncStatus("error", "Błąd zapisu");
    const reason = error?.message ? ` Powód: ${error.message}` : "";
    showToast(`Nie udało się zapisać produktu w chmurze.${reason}`);
    return;
  }
  renderProduct();
  renderProductsList();
