...
function getFilteredProducts() {
  ...
  return products.filter((product) => {
    ...
    return matchesShelf && (!query || searchable.includes(query));
  });
}
...
function openModal(mode = "edit", focusStock = false) {
  ...
  const product = mode === "new"
    ? { name: "", category: "", location: "", description: "", stock: 0, required: 0, ordered: 0, purchasePrice: 0, manufacturer: "", image: "" }
    : getCurrentProduct();
  fillForm(product);
...
function exportProductsToExcel() {
  ...
  const rows = visibleProducts.map((product, index) => ({
    ...
    "Regał": getShelf(product.location),
    "Stan magazynowy": Number(product.stock),
    "Zapotrzebowanie": Number(product.required),
    "Stan / zapotrzebowanie": `${Number(product.stock)}/${Number(product.required)} szt.`,
  ...
  worksheet["!cols"] = [
    { wch: 6 }, { wch: 28 }, { wch: 20 }, { wch: 22 },
    { wch: 16 }, { wch: 10 }, { wch: 16 }, { wch: 17 },
    { wch: 22 }, { wch: 12 }, { wch: 20 }, { wch: 42 }, { hidden: true },
  ];
  worksheet["!autofilter"] = { ref: worksheet["!ref"] };
  visibleProducts.forEach((product, index) => {
    if (Number(product.stock) < Number(product.required)) {
      ["B", "G", "H"].forEach((column) => {
