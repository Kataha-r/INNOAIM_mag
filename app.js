...
const globalClientEmailsInput = document.querySelector("#globalClientEmailsInput");
const saveGlobalClientEmailsButton = document.querySelector("#saveGlobalClientEmailsButton");
const clearGlobalClientEmailsButton = document.querySelector("#clearGlobalClientEmailsButton");
const clientPreviewEmailSelect = document.querySelector("#clientPreviewEmailSelect");
...
function fillForm(product) {
  ...
  form.elements.clientEmails.value = Array.isArray(product.visibleToEmails)
    ? product.visibleToEmails.join(", ")
    : "";
...
function openModal(mode = "edit", focusStock = false) {
  ...
  const product = mode === "new"
    ? { name: "", category: "", machineType: "", location: "A/1/1", description: "", stock: 0, required: 0, ordered: 0, sent: 0, purchasePrice: 0, manufacturer: "", visibleToEmails: [], images: [], shipments: [] }
    : getCurrentProduct();
  fillForm(product);
...
function parseClientEmails(value) {
  return [...new Set(String(value || "")
    .split(/[,\n;]/)
    .map((email) => email.trim().toLocaleLowerCase("pl"))
...
function getAllClientEmails() {
  return [...new Set(products.flatMap((product) =>
    Array.isArray(product.visibleToEmails) ? product.visibleToEmails : []
  ))].sort((a, b) => a.localeCompare(b, "pl"));
}

function productVisibleForClient(product, email) {
  return Array.isArray(product.visibleToEmails)
    && product.visibleToEmails.includes(String(email || "").toLocaleLowerCase("pl"));
}
...
function renderClientsView() {
  ...
  const currentSelection = clientPreviewEmailSelect.value || emails[0] || "";
  globalClientEmailsInput.value = emails.join(", ");
  clientPreviewEmailSelect.innerHTML = emails.length
    ? emails.map((email) => `<option value="${escapeHtml(email)}">${escapeHtml(email)}</option>`).join("")
    : '<option value="">Brak klientów</option>';
  clientPreviewEmailSelect.value = emails.includes(currentSelection) ? currentSelection : (emails[0] || "");

  const selectedEmail = clientPreviewEmailSelect.value;
  const visibleProducts = selectedEmail
    ? products.filter((product) => productVisibleForClient(product, selectedEmail))
    : [];

  clientPreviewSummary.textContent = selectedEmail
    ? `Klient ${selectedEmail} widzi ${visibleProducts.length} z ${products.length} produktów.`
    : "Wpisz e-mail klienta i zapisz dostęp dla produktów.";
  ...
  clientPreviewProductsList.innerHTML = visibleProducts.length
    ...
    `).join("")
    : '<div class="client-empty">Ten klient nie ma jeszcze przypisanych produktów.</div>';
}

async function saveGlobalClientEmailsForAllProducts() {
  const emails = parseClientEmails(globalClientEmailsInput.value);
  const previousProducts = products.map((product) => ({
    ...product,
    visibleToEmails: Array.isArray(product.visibleToEmails) ? [...product.visibleToEmails] : [],
  }));
  products.forEach((product) => {
    product.visibleToEmails = emails;
  });
  ...
  try {
    ...
    showToast(emails.length
      ? `Zapisano dostęp: ${emails.length} klientów widzi wszystkie produkty.`
      : "Wyczyszczono dostęp klientów do produktów."
    );
  } catch (error) {
...
clientPreviewEmailSelect.addEventListener("change", renderClientsView);
saveGlobalClientEmailsButton.addEventListener("click", saveGlobalClientEmailsForAllProducts);
clearGlobalClientEmailsButton.addEventListener("click", () => {
  globalClientEmailsInput.value = "";
  saveGlobalClientEmailsForAllProducts();
});
...
form.addEventListener("submit", async (event) => {
  ...
  const productData = {
    ...
    visibleToEmails: parseClientEmails(data.get("clientEmails")),
