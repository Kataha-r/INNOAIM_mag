...
function renderClientOrder() {
  clientOrderCard.hidden = !clientSession;
...
function exportClientOrderToExcel() {
  ...
  const clientEmail = clientSession?.user?.email || "";
  const orderDate = new Intl.DateTimeFormat("pl-PL", {
    day: "2-digit",
    month: "2-digit",
    year: "numeric",
  }).format(new Date());
  const orderTitle = `Zamówienie klienta ${clientEmail || "bez e-maila"} dnia ${orderDate}`;
...
function setClientSession(session) {
  ...
  clientOrderCard.hidden = !session;
